...
namespace drone
{
    ...
    void Websocket::sendMessageToConnectedClient(const std::string &msg) 
    {
    ...
    {
        if (connected_) {
            sendMessageToConnectedClient(msg);
        }
    }
    
    void Websocket::writeMessage(const std::string& msg, std::promise<bool> &promise) 
    {
        if (connected_) {
